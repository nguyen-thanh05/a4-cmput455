...
MOVE_TYPE = int
WIN = MOVE_TYPE(0)
BLOCK_WIN = MOVE_TYPE(1)
OPEN_FOUR = MOVE_TYPE(2)
CAPTURE = MOVE_TYPE(3)
TYPE_RANDOM = MOVE_TYPE(4)
...
class GtpConnection:
    ...
    def genmove_cmd(self, args: List[str]) -> None:
        ...
        with cProfile.Profile() as pr:
            self.genmove_cmd_temp(args)
            pr.dump_stats("output%i.pr" % self.i)
            stream = open("output%i.txt" % self.i, "w")
            stats = pstats.Stats("output%i.pr" % self.i, stream=stream)
    ...
    def genmove_cmd_temp(self, args: List[str]) -> None:
        """ 
        Modify this function for Assignment 2.
        """
        board_color = args[0].lower()
        color = color_to_int(board_color)
        end_game, result = self.board.is_terminal()
        if end_game:
            ...
            move = self.engine.get_move(self.board, color)
    ...
    def policy_moves_cmd(self, args: List[str]) -> None:
        if self.policy == RANDOM:
            ...
            for point in move_list:
                ...
                move_str_list.append(format_point(move_coord).lower())

            move_str_list.sort()
            for string in move_str_list:
                move_str += " " + string

            self.respond("Random"+move_str)
            return
        
        elif self.policy == RULE_BASED:
            move_list = self.engine.get_moves(self.board)
            move_str_list = []
            move_str = ""
...
def move_type_to_str(move_type:MOVE_TYPE) -> str:
    if move_type == WIN:
        return "Win"
    if move_type == BLOCK_WIN:
        return "BlockWin"
    if move_type == OPEN_FOUR:
        return "OpenFour"
    if move_type == CAPTURE:
        return "Capture"
    if move_type == TYPE_RANDOM:
        return "Random"
